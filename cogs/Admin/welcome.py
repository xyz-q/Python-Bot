...
class ChannelSetupModal(discord.ui.Modal, title="Channel Setup"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("Please enter valid channel IDs!", ephemeral=True, delete_after=8)
        except discord.NotFound:
            await interaction.response.send_message("One or both channels not found!", ephemeral=True, delete_after=8)
...
class WelcomeMessageModal(discord.ui.Modal, title="Welcome Message Setup"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        guild_id = str(interaction.guild.id)
        if guild_id not in self.cog.settings:
            await interaction.response.send_message("Please setup channels first!", ephemeral=True, delete_after=8)
        ...
        await interaction.response.send_message(embeds=[confirmation_embed, preview_embed], ephemeral=True, delete_after=8)
